...
COPY PriceAdmin/PriceAdmin.csproj PriceAdmin/
COPY Shared/Shared.csproj Shared/


# copy and publish app and libraries
COPY PriceAdmin PriceAdmin/
COPY Shared Shared/
WORKDIR /build/PriceAdmin
RUN dotnet restore -r linux-musl-x64
RUN dotnet list PriceAdmin.csproj package
